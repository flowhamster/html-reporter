...
Template.velocityReports.helpers({
  ...
  suites: function () {
    ...
    if (allReports.length > 0) {
      var walkAncestors = function(test, name, ancestors, node) {
        if(!name) {
          return
        }
        if(ancestors.length == 0) {
          ...
          if(!node[ancestor]) {
            node[ancestor] = {name: ancestor}
          }
          walkAncestors(test, name.split(ancestor)[1], test.ancestors, node[ancestor])
        }
      }
      allReports.map(function(doc) {
        walkAncestors(doc, doc.name, doc.ancestors, collection)
      })
      return collection
    }
  },
...
Template.velocityTestReportTree.helpers({
  name: function() {
  ...
  , children: function() {
    var children = []
    Object.keys(Template.instance().data).map(function(key) {
      if(key != 'name') {
        children.push(Template.instance().data[key])
      }
    })
    return children
  }
